...
use AIProjectScanner\Detector\ModuleDetector;
...
$result = (new ModuleDetector())->detect($scanResult);

echo 'Detected Modules:' . PHP_EOL;
echo PHP_EOL;

if ($result->getModules() === []) {
    echo '- None detected' . PHP_EOL;
    exit(0);
}

foreach ($result->getModules() as $module) {
    echo '- ' . $module->getName() . ' (' . count($module->getFiles()) . ' files)' . PHP_EOL;
}